    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                # self is the container, so it should become place.ant
                original_ant = place.ant
                place.ant = self
                self.store_ant(original_ant)
            elif place.ant.can_contain(self):
                # place.ant is the container, so it remains as place.ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        '''def find_bees(obj:Place):
            if hasattr(obj, "is_hive"):
                if obj.is_hive:
                    return None
                else:
                    if hasattr(obj, "bees"):
                        if len(obj.bees) == 0:
                            if hasattr(obj, "entrance"):
                                return find_bees(obj.entrance)
                            else:
                                return None
                        else:
                            return random_bee(obj.bees)
                    else:
                        return None
            else:
                if hasattr(obj, "bees"):
        ...
        '''
        if not hasattr(self, "upper_bound"):
            place = self.place
            def find_bee(obj: Place):
                if obj.is_hive:
                    return None
                if obj.bees == [] and obj.entrance != None:
                    return find_bee(obj.entrance)
                return random_bee(obj.bees)
            return find_bee(place)
        ...
        distance = 0
        while True:
            if place is None or place.is_hive or distance > upper:
                return None
            if distance < lower:
                distance += 1
                place = place.entrance
                continue
            if lower<= distance <= upper and place.bees != []:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if current_place is not None:
            bees_to_damage = current_place.bees[:]
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # super().action(gamestate)
        "*** YOUR CODE HERE ***"
        '''modifying_place = self.place
        while modifying_place is not None:
            if modifying_place.ant is not None and not hasattr(modifying_place.ant,'queen_flag'):
                modifying_place.ant.double()
                # Also double contained ant if it exists (for ContainerAnts)
                if hasattr(modifying_place.ant, 'ant_contained') and modifying_place.ant.ant_contained is not None:
                    modifying_place.ant.ant_contained.double()
                
            modifying_place = modifying_place.exit
        '''

        # First do the normal ThrowerAnt action (throw a leaf)
        super().action(gamestate)
        
        # Then double the damage of all ants behind her in the tunnel
        current_place = self.place.exit  # Start from the place behind the queen
        while current_place is not None:
            if current_place.ant is not None:
                # Double the main ant in this place
                current_place.ant.double()
                
                # Also double contained ant if it exists (for ContainerAnts)
                if hasattr(current_place.ant, 'ant_contained') and current_place.ant.ant_contained is not None:
                    current_place.ant.ant_contained.double()
            
            # Move to the next place behind (towards the end of tunnel)
